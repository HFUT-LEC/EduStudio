...
class R2M_Eedi_20_T12(BaseRaw2Mid):
    ...
    def process(self):
        super().process()
        # 读入数据 查看
        df = pd.read_csv(f"{self.rawpath}/train_data/train_task_1_2.csv",encoding='utf-8')
        df_exer = pd.read_csv(f"{self.rawpath}/metadata/answer_metadata_task_1_2.csv", encoding='utf-8')
        df_qus = pd.read_csv(f"{self.rawpath}/metadata/question_metadata_task_1_2.csv", encoding='utf-8')
        df_user = pd.read_csv(f"{self.rawpath}/metadata/student_metadata_task_1_2.csv", encoding='utf-8')
        ...
        df = df.rename(columns = {'UserId' : 'stu_id:token', 'SchemeOfWorkId':'assignment_id:token_seq',
                                  ...
                                  'timestamp':'start_timestamp:float','Gender':'gender:float','SubjectId':'cpt_seq:token_seq',
        ...
        df_cpt = df_cpt.rename(columns={'SubjectId':'cpt_id:token', 'ParentId':'parent_id:token','Level':'level:float'})
