...
class R2M_Algebra_0506(BaseRaw2Mid):
    ...
    def process(self):
        ...
        # 读取数据集，并展示相关数据
        raw_data_train = pd.read_table(f"{self.rawpath}/algebra_2005_2006_train.txt", encoding='utf-8')
        raw_data_test = pd.read_table(f"{self.rawpath}/algebra_2005_2006_master.txt", encoding='utf-8')
        ...
        def concat_func(x):
            return pd.Series({
                ...
                'correct': x['Corrects'].min(),  # 我们认为只要学生步骤中有一布没有作对，那么整个题就不对
                'KC(Default)': '~~'.join(str(id) for id in x['KC(Default)'])  # 遇到none怎么办？
            })
        ...
        # 最终处理，删除掉旧的stu_id,exer_id,将new_stu_id和new_exer_id修改为stu_id和exer_id
        ...
        # 对KC(Default)进行预处理，将其转化为字符串类型的set集合
        ...
        # 对exercise的初步处理,主要将KC(Default)处理成cpt_seq序列
        ...
        exer['cpt_seq_name'] = exer.apply(lambda x: preprocess_cpt_seq(x['KC(Default)']), axis=1)
