...
class KTInterDataFmtTimeCptUnfold(KTInterDataFmt):
    ...
    def construct_df2dict(cls, cfg, df: pd.DataFrame, maxlen=None):
        ...
        tmp_df = df[['stu_id', 'exer_id', 'cpt_unfold_seq', 'label', 'timestamp']].groupby('stu_id').agg(
            lambda x: list(x)
        ).reset_index()
        ...
        time_lag_seq, _, _ = PadSeqUtil.pad_sequence(
            tmp_df['timestamp'].to_list(), dtype=np.float32,
            maxlen=maxlen
        )
