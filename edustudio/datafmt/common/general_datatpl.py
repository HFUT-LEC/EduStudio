...
class GeneralDataTPL(BaseDataTPL):
    ...
    def __init__(
        ...
        if self.datafmt_cfg['load_data_from'] == 'cachedata':
            self.load_cache()
            self.check_cache()
            self.logger.info(f"Load from cache successfully: {self.datafmt_cfg['cache_id']}")
            self.logger.info(self.datafmt_cfg['dt_info'])
        else:
            # self.feat2type = {}
            self.process_data()
            self.df2dict()
            if self.datafmt_cfg['is_save_cache']:
                self.save_cache()
            self.logger.info(self.datafmt_cfg['dt_info'])
    ...
    def from_cfg(cls, cfg):
        ...
        load_data_from = cfg.datafmt_cfg['load_data_from']
    ...
    def common_str2df(self):
        return {
            ...
            "df_test": self.df_test, "dt_info": self.datafmt_cfg['dt_info']
        }
    ...
    def process_load_data_from_middata(self):
        ...
        if self.datafmt_cfg['is_dataset_divided']:
            raise NotImplementedError # 统计数据集

    def process_data(self):
        load_data_from = self.datafmt_cfg['load_data_from']
    ...
    def save_cache(self):
        ...
        with open(f"{self.cache_folder_path}/datafmt_cfg.json", 'w', encoding='utf-8') as f:
            json.dump(json.loads(self.datafmt_cfg.dump_fmt()), fp=f, indent=2, ensure_ascii=False)

    def check_cache(self):
        with open(f"{self.cache_folder_path}/datafmt_cfg.json", 'r', encoding='utf-8') as f:
            cache_datafmt_cfg = json.load(f)
        
        temp_cache_datafmt_cfg = copy.deepcopy(cache_datafmt_cfg)
        del temp_cache_datafmt_cfg['dt_info']
        del temp_cache_datafmt_cfg['load_data_from']
        # del temp_cache_datafmt_cfg['raw2mid_op']
        # del temp_cache_datafmt_cfg['mid2cache_op_seq']
        curr_datafmt_cfg = copy.deepcopy(json.loads(self.datafmt_cfg.dump_fmt()))
        del curr_datafmt_cfg['dt_info']
        del curr_datafmt_cfg['load_data_from']
        # del curr_datafmt_cfg['raw2mid_op']
        # del curr_datafmt_cfg['mid2cache_op_seq']
        diff = DeepDiff(temp_cache_datafmt_cfg, curr_datafmt_cfg)

        if len(diff) != 0:
            raise ValueError(f"check cache error: {diff}")
        
        self.datafmt_cfg['dt_info'] = cache_datafmt_cfg['dt_info'] # partial load_cache
    ...
    def build_datasets(self):
        n_folds = self.datafmt_cfg['n_folds']
    ...
    def _check_params(self):
        super()._check_params()
        assert self.datafmt_cfg['load_data_from'] in {'rawdata', 'middata', 'cachedata'}
        assert 'dt_info' not in self.datafmt_cfg
        assert self.datafmt_cfg['n_folds'] > 0
    ...
    def cache_folder_path(self):
        save_cache_id = self.datafmt_cfg['cache_id']
    ...
    def _get_r2m_op(cls, cfg):
        r2m_op = cfg.datafmt_cfg['raw2mid_op']
    ...
    def _get_m2c_op_list(self):
        m2c_op_list = self.datafmt_cfg['mid2cache_op_seq']
        op_list = []
    ...
    def df2dict(self):
        # if self.datafmt_cfg['is_dataset_divided'] is True:
